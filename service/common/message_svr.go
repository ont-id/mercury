...
import (
	...
	"github.com/ont-id/mercury/common/packager"
...
func (m *MsgService) SendMsg(msg OutboundMsg) {
	...
	log.Infof("SendMsg messageType:%d,url:%s", msg.Msg.MessageType, url)
	var sendData []byte
	if m.enableEnvelop {
...
func (m *MsgService) GetNextRouter(routers []string) (string, error) {
	...
	if len(routers) < 1 {
		return "", fmt.Errorf("GetNextRouter routers len:%d", len(routers))
	}
